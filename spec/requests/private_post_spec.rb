...
RSpec.describe "Post with authenthication", type: :request do
  ...
  let!(:otauth_headers) {{ 'Authorization' => "Bearer #{other_user.auth_token}"}}
  ...
  describe "POST /posts" do 
    ...
    context "With valid auth" do
      ...
      context "payload" do
        subject { payload }
        it {is_expected.to include(:id, :title, :content, :published, :user_id)}
      end
      context "response"  do
        subject {response}
